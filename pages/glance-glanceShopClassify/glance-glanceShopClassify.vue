...
<template name="glanceShopClassify">
	<view class="glance-shop-classify">
		...
		<!-- 左侧列表占位 -->
		<view style=" float: left;width: 20%;height: 100%; position:fixed;z-index: 100;border-right: #F5F5F5 1px solid;">
			<!-- 左侧主分类列表 -->
			<view v-for="(item,index) in maintitels" :key='index'>
				...
				<view style="text-align: center;line-height: 15px;width: 93%;font-size: 13px; " :class="[activemaintitel == maintitels[index] ? 'glance-shop-classify-left-item-activetxt':'']"> {{ maintitels[index] }}</view>
				</view>
			</view>
		</view>
		<!-- 右侧列表 -->
		<view style="margin-left: 20%;width: 100%;" v-if="activemaintitel">
			<!-- 右侧顶部广告图 -->
			<view style="padding: 15px 15px 5px 15px; background-color: #ffffff; position: sticky;top: 0px;width: 100%;z-index: 10;">
			...
			<scroll-view scroll-y= "true" scroll-left="0" :scroll-top="scrollTop" scroll-with-animation="true" style="background-color: #ffffff;">
				<view class="glance-shop-classify-right-item">
					<view v-for="(item,index) in activesubitems" :key='index' style="width: 95%;">
						<!-- 右侧子分类标题 -->
						<view style="height: 25px;font-size: 15px;text-align: left;border-bottom: solid 1px #F5F5F5;margin-bottom: 20px;">{{ item.subtitel }}</view>
						<!-- 右侧子分类项目图文 -->
						...
						<view v-for="(subitem,k) in item.subitem" :key='k' style="width:33.33%;">
							<image :src="subitem.itemimgsrc" style="height: 100px; width:100%;" @click="clickitemhref(subitem.itemimghref)"></image>
							<view style="height: 20px;line-height: 20px; font-size: 12px;text-align: center;margin-bottom: 20px;">{{ subitem.itemtitel }}</view>
						</view>
						</view>
					</view>
				</view>
	...
	export default {
		...
		onLoad:function(){
			...
			this.data = [
							{
								'maintitel':'居家生活',
								'bannerimgsrc':'../../static/5.jpg',
								'bannerimghref':'点击了推荐专区',
								'subtitel':'季末热卖',
								'itemimgsrc':'../../static/1.jpg',
								'itemtitel':'羽绒厚被',
								'itemimghref':'点击了羽绒厚被'
								},
								{
...
.glance-shop-classify-left-item{
	padding: 10px 0 15px 0; height: 15px; line-height: 15px;text-align: center; width: 100%; font-size: 13px;display: flex;
	display: -webkit-flex;
	flex-flow: row nowrap;
	align-items: center ;
	justify-content: flex-start;
...
.glance-shop-classify-left-item-activetxt{-webkit-transform: scale(1.1);transform: scale(1.1);color: #f40;}
</style>
